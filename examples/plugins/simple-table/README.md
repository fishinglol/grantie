...
- Permissions: **Draw its own blocks inside your notes** (`editor.blocks`), **See what you type on an empty line and what you
  paste** (`editor.input`: the entry in the `//` list and the paste of spreadsheet cells) and **Change the open note**
  (`editor.write`, only for the "Insert a simple table" command). It never reads your other notes and has no network.
