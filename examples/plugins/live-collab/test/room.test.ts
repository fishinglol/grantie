...
test("sealed data opens only for the same room and key, and every seal differs", async () => {
  ...
  const a = await seal(key, "room-one", 1, plain);
  const b = await seal(key, "room-one", 1, plain);
  assert.notDeepEqual([...a], [...b], "a fresh random iv each time");
  assert.equal(new TextDecoder().decode(await open(key, "room-one", 1, a)), "Plan the trip");
  await assert.rejects(open(other, "room-one", 1, a), "another key");
  await assert.rejects(open(key, "room-two", 1, a), "moved to another room");
  await assert.rejects(open(key, "room-one", 4, a), "relabelled as another type by the relay");
  const damaged = a.slice();
  damaged[damaged.length - 1]! ^= 1;
  await assert.rejects(open(key, "room-one", 1, damaged), "tampered with");
});
