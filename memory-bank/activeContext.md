...
- **Left:** merge PR #9 → `npm run deploy` in `apps/docs` (so `https://granite-docs-phi.vercel.app/privacy` is live) → Branding: App name Granite, homepage `https://granite-docs-phi.vercel.app/`, privacy `https://granite-docs-phi.vercel.app/privacy`, no logo → Audience → Publish app (check what Google asks for `vercel.app` domains) → rebuild the desktop app (`tauri build`; env is baked in at build) and ship the phone (`npm run ship`, an OTA JS update is enough for the new client ID) → test a sign-in with a non-test Google account.
