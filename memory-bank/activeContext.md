...
Decision: publish the app to production (`drive.file` is a non-sensitive scope, no security assessment). User moved to a **new Google Cloud project `granite-509815`** (the old one was `colony-497015`, app name "Colony.ai").
- Drafted `apps/docs/privacy.md` (+ footer link in `.vitepress/config.mts`), checked against the code (no analytics; only Google, the install counter, Expo updates, plugins). **Not committed / not deployed yet**: the user should read it first; the contact is `putamafais@gmail.com`; it does not promise Live Collab encryption (that plugin is "SOON" and its crypto is unreviewed).
- The new project has **no Google Auth Platform config yet** (checked: Clients page says "not configured"). To do: enable the Drive API, configure the consent screen (name "Granite", support email, homepage + privacy links), create two OAuth clients ("Desktop app" for `apps/desktop/.env` `VITE_GOOGLE_CLIENT_ID/SECRET`, "TVs and Limited Input devices" for `apps/mobile/.env` `EXPO_PUBLIC_GOOGLE_CLIENT_ID/SECRET`; secrets are shown once, never paste them in chat or commit them), rebuild/ship the apps, then Publish.
- Uploading a logo makes Google require brand verification (domain ownership through Search Console; unclear for a `*.vercel.app` domain) — the logo files are in `Logo/` (`logo-128.png` fits). Homepage: **decided (2026-09-27) = the docs site root** `https://granite-docs-phi.vercel.app/`, which now has an "About Granite" section + a privacy-policy link (`grantie.vercel.app` is a separate project on a different domain, so it can't be the homepage next to `/privacy`). The docs site is NOT Git-connected: publish with `npm run deploy` in `apps/docs` (vercel build + prebuilt deploy).
- Sign-in tokens saved for the old client/project stop working with new client IDs: everyone signs in again once.
- **Done in `granite-509815` (2026-09-27):** Drive API enabled; Google Auth Platform configured (name "Granite", External, contact `putamafais@gmail.com`); Data access = `userinfo.email`, `userinfo.profile`, `openid`, `drive.file` (all non-sensitive); OAuth clients "Granite Desktop" (Desktop app) and "Granite Phone" (TVs and Limited Input devices) created and written into the git-ignored `apps/desktop/.env` and `apps/mobile/.env` (old colony values kept only in the session scratchpad; the old clients still exist in `colony-497015`). Publishing status is still **Testing**.
- **Published (2026-09-27):** PR #9 merged, docs deployed (`https://granite-docs-phi.vercel.app/privacy` and the home page are live), Branding saved (name Granite, homepage = docs root, privacy = `/privacy`, authorised domain `granite-docs-phi.vercel.app` accepted by Google with no error, no logo, no ToS link), and the app was **pushed to production** in `granite-509815` (Audience shows "In production", "Back to testing" reverts it). Google's dialog says verification is only needed with a logo, more than 10 domains or sensitive/restricted scopes: none apply. The consent page for the desktop client shows "Granite" + privacy/ToS links and no `invalid_client`. **Not tested:** a real sign-in with a non-owner Google account (needs the new builds).
- **Update 2026-09-27 (night):** desktop release build installed to `/Applications/Granite.app` (replaced the 2026-09-26 build; it is unsigned like the old one; the build contains the user's uncommitted desktop WIP). Phone: two OTA updates shipped to channel `preview`; the FIRST one crashed the app (see `techContext.md` "OTA updates can't add native modules"), the second (`stores.ts` fix) runs fine on the user's Samsung SM-A356E (checked with adb logcat: no fatals, JS started, process alive 2+ min). Phone sign-in is still the OLD Google client until the user signs out and in again (device-code flow, new client).
- **Left (older list):** desktop release build (`npm run tauri build -- --bundles app`, env baked in; the built frontend was checked to contain the new client ID and not the old one), phone: `npm run ship` in `apps/mobile` (OTA JS update carries the new `EXPO_PUBLIC_*` client; ask the user first, it reaches real devices), then someone signs in with a non-owner Google account on both apps. Optional later: logo (`Logo/logo-128.png`) needs brand verification (Search Console ownership of the domain), a Terms of Service page, a proper app homepage.
