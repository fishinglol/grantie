...
- **Left:** desktop release build (`npm run tauri build -- --bundles app`, env baked in; the built frontend was checked to contain the new client ID and not the old one), phone: `npm run ship` in `apps/mobile` (OTA JS update carries the new `EXPO_PUBLIC_*` client; ask the user first, it reaches real devices), then someone signs in with a non-owner Google account on both apps. Optional later: logo (`Logo/logo-128.png`) needs brand verification (Search Console ownership of the domain), a Terms of Service page, a proper app homepage.
